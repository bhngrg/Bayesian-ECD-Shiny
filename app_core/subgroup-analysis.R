                          ...
                          subgroup_list = NULL, use_pred = TRUE, conf_int = 0.95)
{
  ...
  if(!is.list(subgroup_list))
  {
    ...
    Please input a subgroup specfication list in this form:
         ...
         will examine both female (sex = 0) and male (sex = 1) patients 
         who have been recieved a biopsy (eor = 0),
         and have a kps of less than or equal to 60 (kps = 0) or
         they have a kps of greather than 80 but less than or equal to 100 (kps = 2)
         who are greater than or equal to 48.00 years old 
         and are less than 65.00 years old.
         
         If there is group you don't want to subset on then specify as NA such as:
         list(sex = NA, eor = \"01\", kps = NA, age = NA) would only look at
         patients who have recieved a biopsy (eor = 0) or
         patients who have recieved a subtotal resection (eor = 1).")
  }
  
  if(length(subgroup_list) != length(c(result$Variable_specifications$cont_vars, 
                                       result$Variable_specifications$cat_vars)) || 
     !all(names(subgroup_list) %in% c(result$Variable_specifications$cont_vars, 
                                      result$Variable_specifications$cat_vars)))
  {
    ...
    stop("Please input a subgroup specfication list in either of these form:
         ...
         and have a kps of less than or equal to 60 (kps = 0) or
         they have a kps of greather than 80 but less than or equal to 100 (kps = 2)
         who are greater than or equal to 48 years old 
         and are less than 65 years old.
    ...
    if (length(cat_vars) > 0) {
      ...
      cat_ind_list <- lapply(cat_vars, function(v) {
        ...
        if (is.factor(x)) {
          ...
          # fallback for labeled character columns
          if (v == result$Variable_specifications$cat_vars[1]) {
            # sex
            x_code <- dplyr::case_when(
              x == "Female" ~ 0L,
              x == "Male"   ~ 1L,
              TRUE ~ NA_integer_
            )
          } else {
            stop("Categorical variable ", v,
                 " is character. Please convert it to factor or numeric before analysis.")
          }
        } else {
          stop("Categorical variable ", v,
               " must be factor, numeric, integer, or supported character labels.")
        }
