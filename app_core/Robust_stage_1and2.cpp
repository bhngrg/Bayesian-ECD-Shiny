...
inline double lnpoch_cpp(const double a, const double b) {
  return std::lgamma(a + b) - std::lgamma(a);
}

inline arma::uword sample_categorical_cpp(const arma::vec &prob) {
  const double total = arma::accu(prob);

  if (!std::isfinite(total) || total <= 0.0) {
    return static_cast<arma::uword>(
      std::floor(arma::randu<double>() * prob.n_elem)
    );
  }

  const double u = arma::randu<double>(arma::distr_param(0.0, total));
  const arma::uvec selected = arma::find(arma::cumsum(prob) >= u, 1, "first");

  return selected.is_empty() ? prob.n_elem - 1 : selected(0);
}
...
inline arma::vec rdirichlet_cpp(const arma::vec &alpha) {
  ...
  for (arma::uword i = 0; i < alpha.n_elem; ++i) {
    const double a =
      (std::isfinite(alpha(i)) && alpha(i) > 0.0) ? alpha(i) : 1e-12;

    out(i) = arma::randg<double>(arma::distr_param(a, 1.0));
  }

  const double total = arma::accu(out);
...
double surv_fn_lognorm(const double x, const unsigned nu,const  double ss_survtime,const  double survtime,
  ...
  if(n){
    survtime_j = survtime ;
    mean_j= survtime_j/ n  ;
    ss_j= ss_survtime - n *((mean_j) * (mean_j));
  } else ss_j=  survtime_j = mean_j= 0.0;


  mu_post = (df0*mu0 + survtime_j) / df_post;
  beta_post=beta0+  (ss_j   + tmp* ((mean_j - mu0) * (mean_j - mu0)) )/2.0; //ss_j differently defined from the rest of the code
  ...
  // Rcpp::Rcout<< " ss_j= " << ss_j<< " ((mean_j - mu0) * (mean_j - mu0))= " <<((mean_j - mu0) * (mean_j - mu0))<<" n="<<n << " tmp="<<tmp<< " df_post="<<df_post<< " alpha_post="<<alpha_post<<
...
double post_t_dens(const double x, const  double ss_survtime,const  double survtime,
  ...
  /*double df_final=2.0*alpha_post;
   double denss=lnpoch_cpp(alpha_post, 0.5) - (std::log(arma::datum::pi) / 2.0 + log( sqrt(df_final)* sigma_post ) +(alpha_post+.5)* log1p( (( (x-mu_post)/sigma_post ) * ( (x-mu_post)/sigma_post ))/df_final  ) );
...
arma::vec sim_lognorm_params( const  double ss_j,const  double survtime_j,
  ...
  double beta_post=beta0+  (ss_j -  ((double) n)  * ((mean_j ) * (mean_j ))  + tmp* ((mean_j - mu0) * (mean_j - mu0)) )/2.0;
  double sigma_post =sqrt( beta_post  / (alpha_post * df_post ) );

  arma::vec ret(2);
  // Rcpp::Rcout<<"Flag lognormal_param 0 alpha_post= "<<alpha_post<<" beta_post= "<< beta_post<<" data beta= "<<(ss_j -  ((double) n)  * ((mean_j ) * (mean_j ))  + tmp* ((mean_j - mu0) * (mean_j - mu0)) )/2.0<<endl;
...
double logpi_lognorm_extend(const arma::mat &nj_val,
  ...
  double sum= - (((mu0-mu_m) * (mu0-mu_m))/mu_v + (( log_b0 - b_m) * ( log_b0 - b_m)) / b_v) /2; //normal prior
  ...
  for (unsigned j = 0; j < nj_val.n_rows; ++j)
  {
    if (nj_val(j, t))  // only use this treatment’s column
    {
      ...
      double beta_post = beta0 + (ss_j - nj_val(j, t) * ((mean_j) * (mean_j))
                                    + tmp * ((mean_j - mu0) * (mean_j - mu0))) / 2.0;
...
double ll_alpha(const double l_alpha, const unsigned K, const arma::vec &nj_vec,
  ...
  const double alpha = std::exp(l_alpha);
  ...
  double ret =
    std::lgamma(alpha)
    - std::lgamma(alpha + arma::accu(nj_vec))
    + arma::accu(arma::lgamma(nj_vec + al_by_k))
    - Kn * std::lgamma(al_by_k)
    + log_normpdf(l_alpha, mu_alp, sig_alp);
...
void update_alpha(const unsigned K, const arma::vec &nj_vec, const double mu_alp, const double sig_alp,
  ...
  //get H_ll_new
  double H_new= ll_new+  ((p_lam) * (p_lam)) /2, H_old=ll_old+kin_energy/2;
...
arma::imat stage2_init_alloc_cov_only(
    ...
    const unsigned    rng_seed = 500
){
  (void)rng_seed;  // Retained for R-facing interface compatibility.
...
double logpi_lognorm(const arma::vec &nj_val1, const arma::vec &nj_val2,
  ...
  for(unsigned j=0; j< nj_val2.n_elem; ++j){//Iterate over the clusters
    ...
    if(nj_val2(j)){
      ...
      double beta_post=beta0+  (ss_j -  nj_val2(j) * ((mean_j ) * (mean_j ))  + tmp* ((mean_j - mu0) * (mean_j - mu0)) )/2.0;
      sum+= (a0 *log_b0 -alpha_post *  log( beta_post));
      /*Rcpp::Rcout<<"df_post= "<<df_post<<" tmp="<<tmp<<" beta_post= "<<beta_post<<" ss_j= "<<ss_j<<" nj_val2(j) * ((mean_j ) * (mean_j ))="<<nj_val2(j) * ((mean_j ) * (mean_j ))<<  endl;
      ...
      if(nj_val1(j)){
        ...
        beta_post=beta0+  (ss_j -  nj_val1(j) * ((mean_j ) * (mean_j ))  + tmp* ((mean_j - mu0) * (mean_j - mu0)) )/2.0;
        sum+= (a0 *log_b0 -alpha_post *  log( beta_post));
      }
    }
  }
                                   ...
                                   const int thin)
{
  ...
  double sig_alp = std::sqrt(std::log1p(alpha_hyper(1) / ((alpha_hyper(0)) * (alpha_hyper(0)))));
  ...
  for (unsigned i=0; i < (unsigned)(nrun + burn); ++i){
    ...
    for (unsigned j=0;j<nmix;++j){
      for (unsigned t=0;t<n_trt;++t){
        ...
        for (auto ii : inds_eq_j_shared(j,t)){
          ...
          if (njt > 1){
            ss_j  = ss_survtime(j,t) - ((st(ii)) * (st(ii)));
    ...
    for (unsigned ii=0; ii<n; ++ii){
      ...
      const double st_sq   = ((st(ii)) * (st(ii)));
      const double mu0_t_cur  = params_t(t,0);
      const double beta0_t_cur = std::exp(params_t(t,1));
                                                    ...
                                                    const arma::vec &beta0_control_draws)
{
  ...
  (void)nj_val_old;
  (void)nobs_old;
  (void)lognormal_mu_old;
  (void)lognormal_sig_old;
  ...
  for(int i=0; i<(nrun+burn); ++i){
    ...
    // ================= HEAVY CONSISTENCY CHECK (Stage 2; current cohort only) =================
    {
      ...
      for (unsigned j = 0; j < nmix; ++j) {
        ...
        for (unsigned v = 0; v < k; ++v) {
          arma::uvec cnt(ncat(v), arma::fill::zeros);
          unsigned nobs_re = 0U;
    ...
    if(n_pred){
      ...
      for(unsigned p=0;p<n_pred;++p){
        ...
        for(unsigned t=0;t<n_trt;++t){
          ...
          const double beta0_t_cur = std::exp(params_t(t, 1));
          arma::vec v = sim_lognorm_params(ssC, sC, df0, a0, mu0_t_cur, beta0_t_cur, nC);
          y_pred(p,t) = arma::randn<double>(arma::distr_param(v(0), std::sqrt(v(1))));
        }
      }
...
Rcpp::List common_atoms_cat_lognormal_shared(
  ...
  arma::uvec acceptance_alph(num_cohort, arma::fill::zeros);
  double mu_alp, sig_alp;
  sig_alp = log1p(alpha_hyper(1) / ((alpha_hyper(0)) * (alpha_hyper(0))));
  ...
  for (int i = 0; i < (nrun + burn); ++i) {
          ...
          for (auto jj : inds_eq_j_shared(j,t)) {
            if (!nu(jj)) {
              if (nj_val_shared(j,t) > 1) {
                ss_j      = ss_survtime(j,t) - ((st(jj)) * (st(jj)));
              ...
              beta_post = beta0_t_cur + ( ss_j
                                            - ((double)nj_val_shared(j,t)-1) * ((mean_j) * (mean_j))
                                            + tmp * ((mean_j - mu0_t_cur) * (mean_j - mu0_t_cur)) ) / 2.0;       // [CHG]
                                            ...
                                            st(jj)    = std::min(st(jj), max_st);

                                            ss_survtime(j,t) = ss_j + ((st(jj)) * (st(jj)));
    ...
    for (unsigned jj = 0; jj < n; ++jj) {
      double dens,cluster_prob, pdf_empty, st_sq = ((st(jj)) * (st(jj)));
      log_probs.fill(arma::datum::log_min);
      ...
      if (del(jj) != current_ind) {
        const double st_sq = ((st(jj)) * (st(jj)));
...
Rcpp::List common_atoms_cat_lognormal(const unsigned nmix, arma::uvec ncat,
  ...
  eta.for_each( [&count_cat](arma::umat::elem_type val) { count_cat += (!std::isfinite(val)); } );
    ...
    if(nj_val(j)){
      ...
      for(auto it1:inds_eq_j[j]){//it1 iterates through cluster membership indicators
        ...
        for(auto it2:non_na_obs_cont(it1)){//it2 iterates through non-missing variables of the continuous covariates for each observation
          ...
          ss_j_x(j,it2)+= eta_cont_sq(it1,it2);// (( eta_cont(it1,it2)) * ( eta_cont(it1,it2)));
        }
      }
    }
  }
  ...
  double prob_empty2= log(dir_prec2);
  double df_post, alpha_post, tmp, ss_j, survtime_j, mean_j, mu_post, beta_post, sigma_post;
  ...
  for(int i=0; i<nrun+burn; ++i){
      ...
      if(nj_val1(j)){
        ...
        for(auto jj:inds_eq_j1[j] ){//Iterate over the observations
          if(!nu(jj)){//censored observations
            if(nj_val1(j)>1){
              ss_j= ss_survtime1(j) - ((st(jj) ) * (st(jj) )), survtime_j = survtime1(j)- st(jj);
              mean_j= survtime_j/(nj_val1(j) - 1 );
            } else ss_j=  survtime_j =  mean_j= 0.0;
            mu_post = (df0*mu0 + survtime_j) / df_post;
            beta_post=beta0+  (ss_j -  ((double) nj_val1(j)-1) * ((mean_j ) * (mean_j ))  + tmp* ((mean_j - mu0) * (mean_j - mu0)) )/2.0;
            sigma_post =sqrt( beta_post * (df_post+1)/(df_post *alpha_post) );
            ...
            ss_survtime1(j)=ss_j + ((st(jj)) * (st(jj))); survtime1(j)= survtime_j +st(jj);
          }
        }
      }
      if(nj_val2(j)){
        ...
        for(auto jj:inds_eq_j2[j] ){//Iterate over the observations
          if(!nu(jj)){//censored observations
            if(nj_val2(j)>1){
              ss_j= ss_survtime2(j) - ((st(jj) ) * (st(jj) )), survtime_j = survtime2(j)- st(jj);
              mean_j= survtime_j/(nj_val2(j) - 1 );
            } else ss_j=  survtime_j =  mean_j= 0.0;
            mu_post = (df0*mu0 + survtime_j) / df_post;
            beta_post=beta0+  (ss_j -  ((double) nj_val2(j)-1) * ((mean_j ) * (mean_j ))  + tmp* ((mean_j - mu0) * (mean_j - mu0)) )/2.0;
            sigma_post =sqrt( beta_post * (df_post+1)/(df_post *alpha_post) );
            st(jj)=r_trunclst(2*alpha_post,  mu_post, sigma_post,  st_original(jj), std::numeric_limits<double>::max());
            ...
            /*if(st(jj)>log(100*52))
             Rcpp::Rcout<<"jj="<<jj << " st_original(jj)="<<st_original(jj)<< " st(jj)="<<st(jj)<<endl;*/
            ss_survtime2(j)=ss_j + ((st(jj)) * (st(jj))); survtime2(j)= survtime_j +st(jj);
          }
        }
      }
    }
    ...
    long double log_DEN;
    for(unsigned jj=0;jj<n;++jj){
      double dens, cluster_prob, st_sq=((st(jj)) * (st(jj)));
